package ru.emitrohin.adminui.integration.crud;
...
import ru.emitrohin.adminui.integration.AbstractDatabaseTest;
import ru.emitrohin.adminui.integration.AbstractS3Test;
    ...
    Collection<DynamicTest> authAndUserReadDeleteTest() {
        ...
        return List.of(
                dynamicTest("Стартовый админ авторизуется", () -> {
                    ...
                    AbstractDatabaseTest.log.info("Authorizing admin with jwt token {}", jwtToken);
                ...
                dynamicTest("Читаем данные пользователя по id", () -> {
                    ...
                    AbstractDatabaseTest.log.info("Got user with id {} and telegramId {}", USER_ID, readByIdResponse.getBody().telegramId());
                }),
                dynamicTest("Получить список пользователей из трех", () -> {
                    ...
                    Assertions.assertThat(readAllUsersResponse.getStatusCode()).isEqualTo(HttpStatusCode.valueOf(200)); //ok
                    Assertions.assertThat(readAllUsersResponse.getBody()).isNotNull();
                    Assertions.assertThat(readAllUsersResponse.getBody().size()).as("Размер списка должен быть равен трем").isEqualTo(3);
                    Assertions.assertThat(readAllUsersResponse.getBody()).extracting("firstName").containsExactlyInAnyOrder("John", "Jane", "Alice");
                }),
                dynamicTest("Удалить пользователя по id", () -> {
                    ...
                    Assertions.assertThat(updateResponse.getStatusCode()).isEqualTo(HttpStatusCode.valueOf(204)); //ok
                    Assertions.assertThat(updateResponse.getBody()).isNull();
                }),
                dynamicTest("Получить список пользователей из двух после удаления", () -> {
                    ...
                    Assertions.assertThat(readAllUsersResponse.getStatusCode()).isEqualTo(HttpStatusCode.valueOf(200)); //ok
                    Assertions.assertThat(readAllUsersResponse.getBody()).isNotNull();
                    Assertions.assertThat(readAllUsersResponse.getBody().size()).as("Размер списка должен быть равен двум").isEqualTo(2);
                    Assertions.assertThat(readAllUsersResponse.getBody()).extracting("firstName").containsExactlyInAnyOrder("John", "Jane");
                })
        );
    }
}
